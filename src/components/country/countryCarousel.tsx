...
const CountryCarousel: React.FC = () => {
  ...
  return (
    <div className="bg-[#013D2F] p-20 rounded-[48px] mx-4 sm:mx-8 relative overflow-hidden">
      ...
      <div className="px-6 md:px-12 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          ...
          <div className="flex gap-4">
            ...
            <button
              onClick={nextSlide}
              className="w-12 h-12 rounded-full border bg-accent border-accent flex items-center justify-center text-[#013D2F] hover:bg-white hover:text-[#013D2F] transition-all"
            >
              <ArrowRight size={20} />
            </button>
          </div>
          ...
          {countries.map((country, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Active Card Overlay */}
                {isActive ? (
                  <div className="absolute inset-0 bg-accent/90 p-8 flex flex-col justify-center text-[#013D2F]">
                    <h3 className="text-2xl font-bold mb-4">{country.name}</h3>
                    <p className="text-sm font-medium leading-relaxed mb-6">
                      {country.desc}
                    </p>
                    <button className="flex items-center gap-2 text-sm font-bold border border-[#013D2F] rounded-full px-6 py-3 w-fit hover:bg-[#013D2F] hover:text-white transition-colors">
                      Apply Now <ArrowRight size={14} />
                    </button>

                    {/* Decorative circle */}
                    <div className="absolute top-6 right-6 w-12 h-12 border-2 border-[#013D2F]/20 rounded-full"></div>
                  </div>
                ) : (
